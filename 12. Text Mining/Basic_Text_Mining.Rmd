...
The review dataset has two variables: *content* of the review, and *polarity* of the review (0 or 1). To use the _tm_ package we first transform the dataset to a corpus:
```{r}
...
2. Remove numbers
3. Remove punctuation marks and stop words
...
To predict the polarity (sentiment) of a review, we can make use of a pre-compiled list of words with positive and negative meanings ([Source](http://www.cs.uic.edu/~liub/FBS/sentiment-analysis.html), [negative-words.txt](data/negative-words.txt), [positive-words.txt](data/positive-words.txt))
...
Let's remove the actual textual content for statistical model building
```{r}
