...
For regression problems, we use `neuralnet` and add `linear.output = TRUE` when training model. In addition, the response needs to be standardized to $[0, 1]$ interval. It's important to normalize the response. If not, the algorithm may not converge. I chose to use the min-max method and scale the data in the interval $[0,1]$. Other reference online: [[1](https://machinelearningmastery.com/how-to-improve-neural-network-stability-and-modeling-performance-with-data-scaling/)]
...
For classification problems with `neuralnet` you need to code the response to *factor* first. In addition you want to add `type = "class"` for *predict()* function.
...
Bank_nn <- neuralnet(f, data=Bank_train, hidden=c(3), algorithm = 'rprop+', linear.output=T, likelihood = T)
plot(Bank_nn)
```
