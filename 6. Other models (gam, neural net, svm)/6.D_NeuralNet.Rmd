...
For regression problems, we use `neuralnet` and add `linear.output = TRUE` when training model. In addition, the response needs to be standardized to $[0, 1]$ interval. It's important to normalize the response. If not, most of the times the algorithm will not converge. I chose to use the min-max method and scale the data in the interval $[0,1]$.
...
```{r}
mean(ifelse(Bank_train$DLRSN != pred_nn_in, 1, 0))
```
...
```{r}
mean(ifelse(Bank_test$DLRSN != pred_nn_out, 1, 0))
```
