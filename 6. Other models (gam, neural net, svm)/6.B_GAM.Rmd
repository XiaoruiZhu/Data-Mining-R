...
# Bank_data <- read.csv(file = "D:\\Dropbox\\7.Writing Books\\Data-Mining-R\\docs\\lecture\\data\\bankruptcy8017.csv", header=T)
...
# Bank_glm <- glm(Default ~ LTMTA + Log_PRICE + SIGMA + NIMTA + EXRET + CASHMTA + RSIZE + MBE, family=binomial, data=Bank_data)

# Bank_glm_Null <- glm(Default ~ 1, family=binomial, data=Bank_data)
# 
# summary(Bank_glm)
# summary(Bank_glm_Null)

# Pseudo_R2 <- 1-logLik(Bank_glm)/logLik(Bank_glm_Null)

#create gam model
Bank_gam <- gam(Default ~ s(LTMTA)+s(Log_PRICE)+s(SIGMA)+s(NIMTA)+
                  s(EXRET)+s(CASHMTA)+s(RSIZE)+s(MBE), data=Bank_train)
...
```{r echo=TRUE, message = FALSE,warning=FALSE}
#in-sample mse using df 
Bank_gam_mse_train <- Bank_gam$dev/Bank_gam$df.residual 
#Average Sum of Squared Error
Boston_gam.mse.train <- Boston_gam$dev/nrow(Boston_train) 
