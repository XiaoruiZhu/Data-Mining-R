...
```{r}
mean(ifelse(Bank_train$DLRSN != pred_gam_in, 1, 0))
```
...
```{r}
mean(ifelse(Bank_test$DLRSN != pred_gam_out, 1, 0))
```
...
You are already familiar with the credit scoring set. Let's define a cost function for benchmarking testing set performance.
